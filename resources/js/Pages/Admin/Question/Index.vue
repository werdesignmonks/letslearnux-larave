        ...
        <div class="py-8">
            <div class="">
                <div class="flex gap-2 mb-6">
                    <Button :href="route('question.index')" :active="route().current('question.index')">
                        Chapter List
                    </Button>
                    <Button :href="route('question.create')" :active="route().current('question.create')">
                        Add New Chapter
                    </Button>
                </div>
...
<!--                            </div>-->
                            <div class="overflow-hidden rounded-lg border border-[#E5E6E7]">
                                <table class="min-w-full text-sm font-light overflow-hidden">
                                    ...
                                    <tbody>
                                        <tr class="border-dm-border-color border-t" v-for="(question, index) in props.questions">
                                            <td class="whitespace-nowrap px-3 py-4 font-medium text-center">{{ index + 1 }}</td>
                                            <td class="whitespace-nowrap py-4 text-dm-heading-color text-base font-medium">{{ question.title }}</td>
                                            <td class="whitespace-nowrap py-4 text-left px-4">
                                                <Link :href="route('question.edit', question.id)" class="mr-2 inline-block">
                                                ...
                                                <button class="inline-block" @click.prevent="destroy(question.id)">
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
