...
const isLessonCompleted = (lessonId) => {
	...
	return completedLesson !== undefined;
};
	...
	<AuthenticatedLayout>
		<div class="max-w-[1100px] px-[15px] mx-auto">
			...
			<div class="mt-5 mb-[80px]" v-for="(item, index) in props.chapters.data">
				...
				<div class="mt-6">
					<div
						class="group border border-dm-border-color  py-[19px] px-[20px] flex items-center gap-4 rounded-3xl mb-[12px] hover:border-violet-300 hover:bg-purple-50 transition ease-in-out delay-150"
					...
					>
						...
						<div v-else
						     class="border border-dm-border-color text-base font-bold rounded-full w-[32px] h-[32px] flex items-center justify-center group-hover:border-dm-color-primary group-hover:text-dm-color-primary transition ease-in-out delay-150">
							{{ lesson.serial }}
						</div>

						<Link :href="route('roadmap.show', lesson.slug)"
						      class="text-base w-full font-bold text-dm-heading-color tracking-[-0.5px] group-hover:text-dm-color-primary transition ease-in-out delay-150">
							{{ lesson.title }}
						</Link>
					</div>
				</div>
